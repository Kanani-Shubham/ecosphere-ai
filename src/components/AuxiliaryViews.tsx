...
const getClientChatFallbackReply = (messages: any[], userProfile: any) => {
  ...
  if (lastMessage.toLowerCase().includes("eating") || lastMessage.toLowerCase().includes("food")) {
    ...
    lastMessage.toLowerCase().includes("transport") ||
    lastMessage.toLowerCase().includes("car")
  ) {
    ...
    suggestions: [
      {
        ...
        impact: "High Impact",
        co2Saved: 12,
        reason: "Reduces transport emissions by 40% immediately."
      },
      {
        item: "Reduce AC usage",
        impact: "Medium Impact",
        co2Saved: 8,
        reason: "Warms up room 1°C to optimize compressor load."
      },
      {
        item: "Eat more plant-based meals",
        impact: "Medium Impact",
        co2Saved: 6,
        reason: "Bypasses animal farming lifecycle emissions."
      },
      {
        item: "Avoid single-use plastic",
        impact: "Low Impact",
        co2Saved: 4,
        reason: "Reduces municipal solid waste burden."
      }
    ],
    planner: [
  ...
  return (
    <div className="w-full pb-24">
      <AnimatePresence mode="wait">
        ...
        {viewName === "what-if" && (
          <WhatIfView onBack={onBack} onAddCustomActivity={onAddCustomActivity} />
        )}

        {/* VIEW: S19 Carbon Offset */}
        {viewName === "offset" && (
          <CarbonOffsetView onBack={onBack} onAddCustomActivity={onAddCustomActivity} />
        )}
        ...
        {viewName === "energy-analyzer" && (
          <EnergyUsageAnalyzerView onBack={onBack} onAddCustomActivity={onAddCustomActivity} />
        )}

        {/* VIEW: S29 Travel Impact comparison */}
        {viewName === "travel-impact" && (
          <TravelImpactView onBack={onBack} onAddCustomActivity={onAddCustomActivity} />
        )}
        ...
        {/* VIEW: S37 Eco Wallet */}
        {viewName === "wallet" && (
          <EcoWalletView onBack={onBack} onAddCustomActivity={onAddCustomActivity} />
        )}
      </AnimatePresence>
    </div>
  );
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "init",
      sender: "ai",
      text: `Hi ${profile.name}! I am your AI Eco Coach. I can help you reduce your carbon footprint.\n\nHow can I help you today?`,
      timestamp: "11:22 AM"
    }
  ]);
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      if (response.ok && contentType && contentType.includes("application/json")) {
        ...
        console.warn(
          `[Client-side Resilience] Non-JSON or bad response received for Chat (Status: ${response.status}). Activating local chatbot fallback.`
        );
        data = {
      ...
      if (data.warning || data.isFallback) {
        setWarningMessage(
          data.warning ||
            "Our main AI service is packed! Providing intelligent backup assistance instead."
        );
      }
      ...
      console.warn(
        "[Client-side Resilience] Chat connection error. Activating local chatbot fallback.",
        e
      );
      const data = {
        text: getClientChatFallbackReply([...messages, userMsg], profile),
    ...
    <div className="bg-white border rounded-3xl overflow-hidden shadow-sm flex flex-col justify-between min-h-[520px]">
      ...
      <div className="bg-slate-50 border-b p-4 flex justify-between items-center">
        <div className="flex items-center gap-2.5">
          <button
            onClick={onBack}
            className="bg-white px-2.5 py-1.5 border rounded-xl font-bold text-xs text-slate-500"
          >
            ← Back
          </button>
          <div>
        ...
        <div
          className={`w-2.5 h-2.5 rounded-full animate-ping ${warningMessage ? "bg-amber-400" : "bg-emerald-500"}`}
        />
      </div>

      {warningMessage && (
        <div className="bg-amber-50 border-b border-amber-100 p-3 text-[10px] text-amber-700 font-medium flex items-center gap-2">
          <span className="font-extrabold uppercase bg-amber-200 text-amber-800 rounded px-1 text-[8px]">
            Notice
          </span>
          <span>{warningMessage}</span>
          <button
            onClick={() => setWarningMessage(null)}
            className="ml-auto font-black cursor-pointer hover:text-amber-900"
          >
            ×
          </button>
        </div>
      ...
      <div className="flex-grow p-4 space-y-4 overflow-y-auto max-h-[350px]">
        {messages.map((m) => (
          ...
          >
            <div
              className={`p-3.5 rounded-2xl max-w-xs text-xs shadow-sm ${
                m.sender === "user"
            ...
            >
              ...
              <span
                className={`text-[8px] mt-1.5 block text-right ${m.sender === "user" ? "text-slate-100" : "text-slate-400"}`}
              >
                {m.timestamp}
              </span>
            </div>
      ...
      <div className="p-3 bg-slate-50/50 border-t flex gap-2.5 overflow-x-auto whitespace-nowrap">
        <button
          onClick={() => handleSend("How can I reduce my food footprint?")}
          className="bg-white border rounded-lg px-2.5 py-1 text-[10px] text-slate-600 font-bold"
        >
          Food tips
        </button>
        <button
          onClick={() => handleSend("Give me ways to commute cleaner.")}
          className="bg-white border rounded-lg px-2.5 py-1 text-[10px] text-slate-600 font-bold"
        >
          Commutes
        </button>
        <button
          onClick={() => handleSend("How does solar panel help?")}
          className="bg-white border rounded-lg px-2.5 py-1 text-[10px] text-slate-600 font-bold"
        >
          Solar info
        </button>
      ...
      <div className="p-4 border-t flex gap-3 bg-white">
        ...
        <button
          onClick={() => handleSend()}
          aria-label="Send"
          className="bg-brand-600 hover:bg-brand-700 text-white p-3 rounded-xl"
        >
    ...
    const fetchInsights = async () => {
      try {
        ...
        if (res.ok && contentType && contentType.includes("application/json")) {
          ...
          console.warn(
            `[Client-side Resilience] Non-JSON or bad response received for Insights (Status: ${res.status}). Activating local backup insights engine.`
          );
        ...
        console.warn(
          "[Client-side Resilience] Insights network connection failed. Mapping local green planner forecasts.",
          e
        );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button
          onClick={onBack}
          className="bg-slate-100 p-2.5 rounded-xl hover:bg-slate-200 text-slate-500 text-xs font-bold font-mono"
        >
          ← Back
        </button>
        <div>
      ...
      {isLoading ? (
        <div className="bg-white rounded-3xl p-8 border text-center text-slate-400">
          ...
          <p className="text-xs font-bold leading-none font-mono">
            Syncing eco trends with Gemini...
          </p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-3xl border p-5 shadow-sm space-y-4">
            <h4 className="font-extrabold text-xs text-slate-400 uppercase tracking-wider">
              Top Suggestions Proposed
            </h4>
            <div className="space-y-3">
              {data?.suggestions?.map((item: any, id: number) => (
                <div
                  key={id}
                  className="p-3 bg-slate-50 rounded-2xl border border-slate-100/50 flex justify-between items-center"
                >
                  <div>
              ...
              {data?.planner?.map((day: any, i: number) => (
                ...
                >
                  <div className="flex gap-3 items-center">
                    <span className="w-8 font-black font-mono text-slate-400 text-xs uppercase text-center bg-slate-50 py-1 rounded-md">
                      {day.day}
                    </span>
...
function WhatIfView({
  ...
  const handleApplyScenario = () => {
    ...
    alert(
      `Applied! This hypothetical choice reduces your footprint metrics by -${potentialSavings} kg CO₂e!`
    );
    onBack();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-medium">
            Verify custom scenario carbon impact
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border p-5 shadow-sm space-y-4">
        ...
        <div className="space-y-3">
          {[
            {
              id: "ev",
              title: "Buy an Electric Vehicle (EV)",
              reduction: "-45 kg CO₂e",
              detail: "Bypasses fossil engines entirely"
            },
            {
              id: "public",
              title: "Adopt Public Transit full-time",
              reduction: "-32 kg CO₂e",
              detail: "Share transit grid occupancy"
            },
            {
              id: "ac",
              title: "Reduce AC thermostat load by 2°C",
              reduction: "-18 kg CO₂e",
              detail: "Optimize compressor utility"
            },
            {
              id: "vegan",
              title: "Transition to fully Vegan diet",
              reduction: "-25 kg CO₂e",
              detail: "Bypass organic cattle farming life cycle emissions"
            },
            {
              id: "flights",
              title: "Reduce regional flights by half",
              reduction: "-40 kg CO₂e",
              detail: "Lesser aviation jet fuel logs"
            }
          ].map((item) => (
            <button
              key={item.id}
              onClick={() =>
                setSwitches((prev) => ({
                  ...prev,
                  [item.id]: !prev[item.id as keyof typeof switches]
                }))
              }
      ...
      <div className="bg-emerald-600 rounded-3xl p-6 text-white shadow-md text-center space-y-4">
        <h4 className="font-extrabold text-xs text-emerald-100 uppercase tracking-widest">
          Aggregate Calculated Savings
        </h4>
        <p className="text-3xl font-black font-display">
          {potentialSavings} kg CO₂e{" "}
          <span className="text-xs font-semibold text-emerald-100">Reduction</span>
        </p>
        <p className="text-xs text-emerald-100 leading-relaxed font-normal">
          If implemented today, you'll reduce your personal monthly carbon emissions by **
          {((potentialSavings / 243) * 100).toFixed(0)}%**!
        </p>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-medium font-mono">
            Compensate direct localized emissions
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border rounded-3xl p-5 shadow-sm space-y-4">
        <div className="bg-gradient-to-tr from-brand-600 to-emerald-500 rounded-2xl p-5 text-white shadow-inner">
          ...
          <p className="text-xs text-emerald-100 mt-2 font-normal leading-relaxed">
            Support certified international reforestation and clean hydro-electric projects to
            offset essential lifestyle commuting loads.
          </p>
        </div>
          ...
          {[
            {
              ...
              cost: 15,
              value: 50,
              desc: "Plants indigenous high-canopy trees in Ecuador to absorb industrial co2 logs.",
              icon: "🌳"
            },
            {
              id: "o2",
              name: "Equator Certified Solar Energy",
              cost: 30,
              value: 120,
              desc: "Supports localized communities with solar microgrid infrastructure.",
              icon: "☀️"
            },
            {
              id: "o3",
              name: "Clean Water Filter Wells",
              cost: 20,
              value: 80,
              desc: "Bypasses fossil boiling sanitation demands in Kenya communities.",
              icon: "💧"
            }
          ].map((item) => (
            <div
              key={item.id}
              className="p-4 bg-slate-50/50 rounded-2xl border border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
            >
              <div className="flex gap-3 leading-tight">
                ...
                <div>
                  <h5 className="font-bold text-slate-800 text-xs flex items-center gap-2">
                    {item.name}
                    <span className="bg-emerald-50 text-emerald-700 text-[9px] px-1.5 py-0.5 rounded border">
                      -{item.value} kg CO₂
                    </span>
                  </h5>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-medium">
            Build Carbon awareness and climate knowledge
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border p-5 shadow-sm space-y-4">
        <div className="bg-sky-600 rounded-2xl p-5 text-white shadow-md">
          <p className="text-[10px] font-bold text-sky-100 uppercase tracking-widest leading-none">
            Education Hub
          </p>
          <h4 className="font-extrabold text-sm mt-1.5">
            Improve climate IQ to unlock achievements
          </h4>
          <p className="text-xs text-sky-100 mt-2 font-normal leading-relaxed">
            Read brief cards on carbon metrics, test your knowledge, of clean habits and offset
            multipliers.
          </p>
        </div>
          ...
          {[
            {
              ...
              difficulty: "Beginner",
              content:
                "A carbon footprint represents the aggregate total volume of greenhouse gases (including carbon dioxide and methane) emitted by our industrial, agricultural, and daily commuting choices."
            },
            {
              id: 2,
              title: "How to Reduce Household Emissions",
              duration: "7 mins",
              difficulty: "Intermediate",
              content:
                "Optimize heating elements, wash laundry on cold off-peak schedules, clean AC ventilation systems twice annually, and switch home energy inputs to solar grids."
            },
            {
              id: 3,
              title: "The Meat Lifecycle Food Impact",
              duration: "6 mins",
              difficulty: "Intermediate",
              content:
                "Red meat farming accounts for over 18% of global greenhouse emissions. Transitioning to organic plant grain proteins reduces meal metrics by up to 75%!"
            },
            {
              id: 4,
              ...
              difficulty: "Advanced",
              content:
                "Offsetting allows individuals to compensate essential travel loads by purchasing equivalent credits that fund verified global clean projects like wind farms and reforestation."
            }
          ].map((lesson) => (
            <div
              key={lesson.id}
              className="border border-slate-100 rounded-2.5xl overflow-hidden bg-slate-50/50"
            >
              <button
              ...
              >
                ...
                <ArrowRight
                  className={`w-4 h-4 text-slate-350 transition-all ${activeLesson === lesson.id ? "rotate-90" : ""}`}
                />
              </button>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-medium">
            Verify 2050 localized environmental paths
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm flex flex-col items-center text-center space-y-6">
        {twinOption === "current" && (
          <div className="space-y-4">
            <div className="w-24 h-24 bg-brand-100 rounded-full flex items-center justify-center text-3xl shadow">
              👤
            </div>
            <h4 className="font-extrabold text-slate-800 text-base leading-none">
              Your Current Self baseline
            </h4>
            ...
            <p className="text-xs text-slate-600 leading-relaxed font-normal max-w-xs">
              Based on your current logged commuting, utility, and diet habits, your sustainable
              Twin footprint is 18% lower than your localized neighborhood average.
            </p>
          </div>
        )}
        ...
        {twinOption === "future" && (
          <div className="space-y-4">
            <div className="w-24 h-24 bg-rose-100 rounded-full flex items-center justify-center text-4xl shadow">
              🌋
            </div>
            <h4 className="font-extrabold text-slate-850 text-base leading-none">
              Your Future Self in 2050 (Unchanged)
            </h4>
            ...
            <p className="text-xs text-slate-500 leading-relaxed font-normal max-w-xs">
              If your current carbon habits continue unchanged under standard city emissions
              escalation models, heating loads and sea level impacts will increase by 28%.
            </p>
          </div>
        )}
        ...
        {twinOption === "green" && (
          <div className="space-y-4">
            <div className="w-24 h-24 bg-indigo-100 rounded-full flex items-center justify-center text-4xl shadow font-display">
              🌎
            </div>
            <h4 className="font-extrabold text-slate-850 text-base leading-none">
              Your Future Self (Green Adaptation)
            </h4>
            <div className="bg-indigo-50 text-indigo-700 font-bold px-4 py-1.5 border rounded-xl text-base leading-none">
              98 kg CO₂e / month (-62%)
            </div>
            <p className="text-xs text-slate-500 leading-relaxed font-normal max-w-xs">
              Adopting clean transport choices and supporting solar microgrids will lower your Twin
              baseline by 62% in the long term, preventing up to 2 tons of thermal CO2 equivalents!
            </p>
          </div>
        )}
      </div>
...
function EnergyUsageAnalyzerView({
  onBack,
  onAddCustomActivity
}: {
  onBack: () => void;
      ...
      <div className="bg-white border rounded-3xl p-5 shadow-sm space-y-4">
        ...
        <div className="space-y-3.5">
          {[
            {
              id: "app-1",
              name: "Domestic Air Conditioner",
              kwh: "12.0 kWh / day",
              co2: 5.4,
              tip: "Thermostat to 25°C"
            },
            {
              id: "app-2",
              name: "Compression Refrigerator",
              kwh: "2.4 kWh / day",
              co2: 1.6,
              tip: "Clean coils for 10% gain"
            },
            {
              id: "app-3",
              name: "High-voltage Boiler",
              kwh: "4.5 kWh / day",
              co2: 2.0,
              tip: "Reduce boiler water heat"
            }
          ].map((app) => (
            <div
              key={app.id}
              className="p-4 bg-slate-50/50 rounded-2xl border border-slate-100 flex justify-between items-center"
            >
              <div>
                <h5 className="font-bold text-slate-800 text-xs">{app.name}</h5>
                <p className="text-[10px] text-slate-400 font-medium mt-1 font-mono uppercase">
                  {app.kwh} • {app.co2} kg CO2e / run
                </p>
              </div>
              {!powerSaved[app.id] ? (
                <button
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-medium font-mono">
            Compare localized travel avenues
          </p>
        </div>
      </div>
          ...
          {travelModes.map((item) => (
            ...
            >
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-0.5 uppercase tracking-wide font-mono font-medium">
                  {item.time} commute
                </p>
              </div>
...
function FamilyDashboardView({ onBack }: { onBack: () => void }) {
  ...
  const totalSavedCo2 = Math.abs(
    activities.reduce((sum, act) => sum + (act.co2Value < 0 ? act.co2Value : 0), 0)
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border rounded-3xl p-6 shadow-sm flex flex-col items-center justify-center text-center space-y-4">
        ...
        <h4 className="font-extrabold text-slate-850 text-base leading-none">
          Your Family Footprint Score
        </h4>
        <p className="text-3xl font-black text-slate-850 font-display">
          {(300 - totalSavedCo2).toFixed(0)} kg{" "}
          <span className="text-xs font-semibold text-slate-400">CO₂e / month</span>
        </p>
        ...
        <div className="w-full text-left space-y-3 pt-4 border-t border-slate-100">
          <p className="font-extrabold text-[10px] text-slate-400 uppercase tracking-widest mb-3">
            Family members rankings
          </p>
          {[
            {
              name: "Anita profile (Mom)",
              saved: "24 kg saved",
              stars: "⭐⭐⭐⭐",
              xp: "1,450 XP"
            },
            {
              name: "Rohan profile (Father)",
              saved: "18 kg saved",
              stars: "⭐⭐⭐",
              xp: "1,120 XP"
            },
            {
              name: `${profile?.name || "My profile"} (You)`,
              saved: `${totalSavedCo2.toFixed(1)} kg saved`,
              stars: "⭐⭐⭐⭐⭐",
              xp: `${(profile?.totalXP ?? profile?.xp ?? 0).toLocaleString()} XP`
            }
          ].map((member, i) => (
            <div
              key={i}
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-medium">
            Department carbon tracking analytics
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border rounded-3xl p-5 shadow-sm space-y-6">
        <div className="flex justify-between items-center p-4 bg-emerald-600 rounded-2xl text-white">
          <div>
            <p className="text-[9px] font-bold text-emerald-100 uppercase tracking-widest leading-none">
              Acme Corp Footprint
            </p>
            <p className="text-2xl font-black mt-1 font-display">
              2,450 kg <span className="text-xs font-semibold text-emerald-100">CO₂e</span>
            </p>
          </div>
      ...
      <div className="bg-white border rounded-3xl p-5 shadow-sm space-y-6">
        ...
        <div className="relative border-l border-slate-100 pl-4 ml-2 space-y-6">
          {[
            {
              date: "TODAY",
              title: "Walked 3 km today",
              desc: "Earned +20 XP. Bypassed vehicle emissions.",
              icon: "🌱"
            },
            {
              date: "MAY 11",
              title: "Completed Energy Appliance Audit",
              desc: "Lowered AC water heating settings.",
              icon: "⚡"
            },
            {
              date: "MAY 10",
              title: "Switched to plant-based meals",
              desc: "Reduced groceries recipe carbon load.",
              icon: "🥗"
            },
            {
              date: "MAY 9",
              title: "Setup EcoSphere AI Profile",
              desc: "Locked baseline metrics and initial data.",
              icon: "🌎"
            }
          ].map((item, id) => (
            <div key={id} className="relative">
              ...
              <div>
                <span className="text-[10px] font-black font-mono text-slate-400 tracking-wider block leading-none">
                  {item.date}
                </span>
                <h5 className="font-bold text-slate-800 text-xs mt-1.5">{item.title}</h5>
                <p className="text-xs text-slate-550 mt-1 font-normal leading-normal">
                  {item.desc}
                </p>
              </div>
            </div>
...
function EcoWalletView({
  ...
  const handleRedeemReward = async (title: string, cost: number) => {
    ...
    alert(
      `Redeemed! Your promotional code has been allocated: ECO-${Math.random().toString(36).substr(2, 6).toUpperCase()}`
    );
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-400 font-medium">
            Verify points balances and prizes
          </p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-tr from-brand-600 to-emerald-500 text-white rounded-3xl p-6 shadow-md text-center space-y-3 relative overflow-hidden">
        ...
        <p className="text-4xl font-black font-display">
          ...
          <span className="text-xs font-semibold text-emerald-100">EcoPoints</span>
        </p>
        <p className="text-xs text-emerald-100 max-w-xs mx-auto leading-normal font-normal">
          Spend points on planting native forest saplings or shopping organic certified sustainable
          options.
        </p>
      </div>
      ...
      <div className="bg-white border rounded-3xl p-5 shadow-sm space-y-4">
        ...
        <div className="space-y-3">
          {[
            {
              title: "Plant a native Cedar tree",
              cost: 1000,
              desc: "Plants a tree sapling in Madagascar with official coordinates."
            },
            {
              title: "10% Organic Grocery voucher",
              cost: 500,
              desc: "Promotional discount supported at partner markets."
            },
            {
              title: "Support 100 watt solar hours",
              cost: 750,
              desc: "Neutralizes appliance logs on the grid database."
            }
          ].map((r, i) => (
            <div
              key={i}
              className="p-3.5 bg-slate-50 rounded-2xl border border-slate-100 flex justify-between items-center gap-4"
            >
              <div>
                <h5 className="font-bold text-xs text-slate-800">{r.title}</h5>
                <p className="text-[10px] text-slate-400 mt-1">{r.desc}</p>
                <span className="text-[10px] font-black text-emerald-600 font-mono mt-1.5 block">
                  {r.cost} Pts required
                </span>
              </div>
              <button
        ...
        <div className="divide-y">
          {transactions.map((tx) => (
            ...
            >
              ...
              <span
                className={`text-xs font-bold font-mono ${tx.type === "earn" ? "text-emerald-600" : "text-rose-500"}`}
              >
                {tx.pts}
              </span>
            </div>
