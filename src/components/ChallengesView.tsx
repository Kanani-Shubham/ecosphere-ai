...
export default function ChallengesView({
  ...
  const [subTab, setSubTab] = useState<"active" | "leaderboard" | "gamification" | "community">(
    "active"
  );
  const [challengeFilter, setChallengeFilter] = useState<"daily" | "weekly" | "special">("daily");
  const [leaderboardFilter, setLeaderboardFilter] = useState<"global" | "friends" | "local">(
    "global"
  );
  ...
  const [preRegisteredJuly, setPreRegisteredJuly] = useState(
    () => localStorage.getItem("ecosphere_prereg_july") === "true"
  );

  // Achievement Sharing dialog states
  const [sharingBadge, setSharingBadge] = useState<Badge | null>(null);
  const [ecoMotto, setEcoMotto] = useState(
  ...
  // Reply input states
  const [nestedReplyTarget, setNestedReplyTarget] = useState<{
    postId: string;
    commentId: string;
  } | null>(null);
  ...
  const handleCommunityPostSubmit = (e: React.FormEvent) => {
    ...
    store.addCommunityPost(newPostText, postCategory, imagePayload || undefined, postType);
  ...
  const getLeaderboardList = () => {
    const me = {
      ...
      xp: totalXP,
      avatar:
        profile.profileImage || "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=100",
      isMe: true,
      type: "all"
    };
      ...
      <AnimatePresence mode="wait">
        ...
        {subTab === "active" && (
          ...
          >
            ...
            <div
              className="bg-gradient-to-br from-indigo-900 via-indigo-950 to-emerald-950 rounded-3xl p-6 text-white shadow-lg space-y-5 relative overflow-hidden border border-indigo-700/30"
              id="seasonal-campaign-dashboard"
            >
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-xl font-black mt-2 font-display">
                    June Sustainability Marathon
                  </h3>
                  <p className="text-[11px] text-slate-300 font-medium leading-normal mt-1">
                    Timeline: June 1st - June 30th, 2026. Focus: domestic waste reduction, climate
                    HVAC settings, and organic regional foods.
                  </p>
                </div>
              ...
              {(() => {
                const JUNE_SEASONAL_CHALLENGES = [
                  {
                    id: "sc-june-1",
                    title: "Zero Single-Use Plastics Marathon",
                    description:
                      "Use only 100% biodegradable packaging containers and refuse any plastic flatware today.",
                    xpReward: 180,
                    pointsReward: 400,
                    category: "waste",
                    difficulty: "Medium"
                  ...
                  {
                    id: "sc-june-2",
                    title: "Power Down HVAC Standby Load",
                    description:
                      "Set household temperature controls to 25°C and disable redundant idle wall-power drains.",
                    xpReward: 150,
                    pointsReward: 350,
                    category: "energy",
                    difficulty: "Easy"
                  ...
                  {
                    id: "sc-june-3",
                    title: "Local Organic Harvest Feast",
                    description:
                      "Prepare a breakfast or dinner consisting solely of local community farm-grown produce.",
                    xpReward: 220,
                    pointsReward: 500,
                    category: "food",
                    difficulty: "Hard"
                ...
                const setClaimedMilestones = (items: any) => {
                  localStorage.setItem(
                    "ecosphere_seasonal_milestones_claimed_v4",
                    JSON.stringify(items)
                  );
                  window.dispatchEvent(new Event("storage"));
                };

                const completedCount = JUNE_SEASONAL_CHALLENGES.filter((sc) =>
                  completedSeasonalIds.includes(sc.id)
                ).length;
                const progressPct = Math.round(
                  (completedCount / JUNE_SEASONAL_CHALLENGES.length) * 100
                );
                ...
                const handleCompleteSeasonal = async (ch: any) => {
                  ...
                  await store.addRewardTransaction(
                    `Seasonal Challenge: ${ch.title}`,
                    ch.pointsReward,
                    ch.xpReward,
                    "challenge"
                  );
                  ...
                  <div className="space-y-4">
                    <div className="bg-white/10 rounded-2xl p-4 border border-white/5">
                      <div className="flex justify-between items-center text-xs font-bold font-mono">
                        ...
                        <span>
                          {completedCount} / {JUNE_SEASONAL_CHALLENGES.length} Completed (
                          {progressPct}%)
                        </span>
                      </div>
                    ...
                    <div className="space-y-2.5">
                      ...
                      {JUNE_SEASONAL_CHALLENGES.map((sc) => {
                        ...
                        return (
                          <div
                            key={sc.id}
                            className="p-3.5 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-between gap-4"
                          >
                            <div>
                              <div className="flex items-center gap-2">
                                <span
                                  className={`text-[8px] font-mono leading-none tracking-wider font-extrabold px-1.5 py-0.5 rounded ${
                                    sc.difficulty === "Easy"
                      ...
                      <div className="grid grid-cols-3 gap-2.5">
                        {[
                          ...
                          return (
                            ...
                            >
                              <p className="text-[8px] font-extrabold uppercase tracking-wide leading-tight">
                                {m.key}
                              </p>
                  ...
                  {showArchives && (
                    ...
                    >
                      {[
                        {
                          id: "s-march",
                          title: "Spring Biosphere Kickoff",
                          period: "March 2026",
                          xp: "420 XP",
                          badge: "Spring Scout",
                          status: "Completed",
                          color: "border-slate-800 bg-[#a7f3d0]/10"
                        },
                        {
                          id: "s-april",
                          title: "Earth Day Action Month",
                          period: "April 2026",
                          xp: "500 XP",
                        ...
                        {
                          id: "s-july",
                          title: "July Green Mobility Challenge",
                          period: "July 2026",
                          xp: "Upcoming",
                          badge: "Mobility Champion Badge",
                          status: "Preregister Selection",
                          color: "border-slate-800 bg-[#a5b4fc]/5"
                        }
                      ].map((h) => (
                        <div
                          key={h.id}
                          className={`p-3 rounded-2xl border text-xs flex justify-between items-center ${h.color}`}
                        >
                          <div>
                            ...
                            <p className="text-[10px] text-slate-300 mt-1">
                              Reward earned: {h.xp} | Badge unlocked: {h.badge}
                            </p>
                          </div>
                          {h.id === "s-july" ? (
                            <button
            ...
            <div className="space-y-3" id="challenges-list">
              {challenges
                .filter((c) => c.type === challengeFilter)
                .map((item) => (
                  <div
                    key={item.id}
                    className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm flex items-start justify-between gap-4"
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span
          ...
          >
              ...
              <div className="flex-grow max-w-sm">
                <div className="flex items-center justify-between text-xs font-bold text-slate-500 mb-1.5">
                  ...
                  <span className="text-slate-400">
                    {(profile.xpNeeded ?? 500).toLocaleString()} XP
                  </span>
                </div>
                <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, Math.max(0, (totalXP / (profile.xpNeeded || 500)) * 100))}%`
                    }}
                  />
                </div>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs text-brand-600 font-bold select-none">
          ...
          <div className="space-y-6" id="community-tab-shell">
            ...
            <div className="bg-white border border-slate-100 p-4 rounded-3xl shadow-sm space-y-3">
              ...
              <div
                ...
                id="community-category-scroller"
              >
                {[
            ...
            <form
              onSubmit={handleCommunityPostSubmit}
              ...
              <textarea
                placeholder={
                  postType === "achievement"
                    ? "Showcase your sustainable level milestones or carbon breakthroughs..."
                    : postType === "milestone"
                      ? "Describe your carbon savings breakthrough for today..."
              ...
              {filteredPosts.length === 0 ? (
                <div className="bg-white border border-slate-100 p-8 rounded-3xl text-center text-slate-400">
                  ...
                  <p className="text-[10px] text-slate-400 mt-1">
                    Be the first to publish your daily milestone story!
                  </p>
                </div>
              ) : (
                filteredPosts.map((post) => {
                  ...
                  return (
                    <div
                      key={post.id}
                      className="bg-white rounded-3xl border border-slate-100 p-5 shadow-sm space-y-4 relative overflow-hidden transition-all"
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span className="text-[9px] font-extrabold text-brand-600 uppercase tracking-wider">
                              {post.category}
                            </span>
                            {post.isEdited && (
                          ...
                          {!isUserPost && (
                            <button
                              onClick={() => {
                                if (
                                  confirm(
                                    "Report this post for inappropriate/irrelevant content? This hides the post immediately."
                                  )
                                ) {
                      ...
                      {isExpanded && (
                        <div
                          className="mt-4 pt-4 border-t border-slate-50 space-y-3.5 transition-all"
                          id={`comments-${post.id}`}
                        >
                          ...
                          <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1">
                            {!post.commentsList || post.commentsList.length === 0 ? (
                              <p className="text-[10px] text-slate-400 italic text-center py-2">
                                No comments published yet. Be the first to express organic thoughts!
                              </p>
                            ) : (
                                      ...
                                      <div className="my-1.5 pl-6 space-y-2">
                                        {comm.replies.map((rep) => {
                                          ...
                                          return (
                                            <div
                                              key={rep.id}
                                              className="bg-emerald-50/30 p-2.5 rounded-xl border border-emerald-100 flex gap-2"
                                            >
                                              <img
                                                src={rAvatar}
                                                alt={rName}
                                              ...
                                              <div className="flex-1">
                                                <div className="flex items-center justify-between gap-2">
                                                  <span className="text-[9px] font-bold text-slate-800">
                                                    {rName}{" "}
                                                    <span className="text-[8px] font-medium text-slate-400 bg-white border border-slate-100 px-1 py-0.2 rounded-full">
                                                      Replied
                                                    </span>
                                                  </span>
                                                  <span className="text-[8px] text-slate-400">
                                        ...
                                        <input
                                          ...
                                          onKeyDown={(e) => {
                                            if (e.key === "Enter" && nestedReplyText.trim()) {
                                              store.replyCommentCommunityPost(
                                                post.id,
                                                comm.id,
                                                nestedReplyText
                                              );
            ...
            >
              ...
              <div className="bg-gradient-to-b from-[#111c16] to-[#0d0d1b] rounded-2xl border border-emerald-500/20 p-5 relative overflow-hidden text-center shadow-xl space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-slate-400 font-bold max-w-[210px] mx-auto leading-normal">
                ...
                <div className="p-3 bg-white/5 rounded-xl border border-white/5 flex items-center gap-2.5 text-left">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[9px] font-mono text-emerald-400 font-extrabold uppercase leading-none">
                      Level {profile.level} {profile.levelRank}
                    </p>
                  </div>
                  <span className="ml-auto text-[8px] font-mono text-slate-500 font-bold border border-slate-800 px-1 py-0.5 rounded">
                    VERIFIED
                  </span>
                </div>
              ...
              <div className="space-y-2">
                ...
                >
                  <option value="Small acts multiplied by millions can transform our planet.">
                    🌱 Small acts multiply, planet heals
                  </option>
                  <option value="Conserving power today safeguards the carbon metrics of tomorrow.">
                    ⚡ Save energy today, protect tomorrow
                  </option>
                  <option value="Pedal power saves fuel, preserves nature, and strengthens lungs!">
                    🚴 Cycling drives clean atmospheric loops
                  </option>
                  <option value="Plant-based diets keep carbon emissions low and biosphere balance high.">
                    🧁 Plant eating preserves forest environments
                  </option>
                </select>
              </div>
