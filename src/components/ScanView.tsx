...
import { 
  Camera, Receipt, FileText, ShoppingBag, Upload, AlertCircle, RefreshCw, 
  Check, ArrowRight, ListCollapse, HelpCircle, ShieldAlert, Trash2
} from "lucide-react";
...
const getClientAnalyzeImageFallback = (type: string) => {
  if (type === "receipt") {
    return {
      ...
      score: 83,
      totalImpact: 1.9,
      summary: "This organic receipt contains a high percentage of sustainable ingredients, though flight-imported items represent 42% of the item carbon profile. (Resilient local estimation mode)",
      isDemo: true,
      isFallback: true
    };
  } else if (type === "bill") {
    return {
      ...
      score: 65,
      totalImpact: 128.4,
      summary: "Your electricity statement reveals high refrigeration heating loss. Clean AC filters twice yearly to reduce load by up to 15%. (Resilient local statement mode)",
      isDemo: true,
      isFallback: true
    };
    ...
    return {
      ...
      score: 95,
      totalImpact: 0.12,
      summary: "This product utilizes 100% compostable bamboo handles and plant-derived bristles. The packaging is plastic-free and biodegradable recycled board. (Resilient local product scanner mode)",
      suggestions: [
    ...
    return {
      ...
      co2: 2.4,
      confidence: 96,
      summary: "This visual captures combustion-engine tailpipe emissions. Standard sedans emit 120-180g CO2 per kilometer traveled. (Resilient local visual mode)",
      suggestions: [
...
export default function ScanView({ onAddCustomActivity, userProfile }: ScanViewProps) {
  ...
  const [activeScantype, setActiveScantype] = useState<"hub" | "receipt" | "camera" | "bill" | "product" | "result">("hub");
  ...
  const handleExportResult = () => {
    ...
    const itemsStr = (scanResult.detectedItems || []).map((item: any) => `- ${item.name}: +${item.co2} kg CO2e (Alternative: ${item.alternative})`).join("\n");
  ...
  const handleScanSubmit = async () => {
    ...
    try {
      ...
      if (response.ok && contentType && contentType.includes("application/json")) {
        ...
        console.warn(`[Client-side Resilience] Non-JSON or bad response received (Status: ${response.status}). Activating local visual fallback estimate engine.`);
        data = getClientAnalyzeImageFallback(selectedType);
      }
      setScanResult(data);

      await addScan({
        name: selectedType === "receipt" ? "Receipt: Organic Groceries" : selectedType === "bill" ? "Utility energy Statement" : selectedType === "product" ? "Product: " + (data.objectDetected || "Consumer Good") : "Carbon Camera: " + (data.objectDetected || "Physical Object"),
        type: (selectedType === "product" ? "camera" : selectedType) as any,
        co2Value: Number((data.totalImpact || data.co2 || 1.9)),
        category: data.category || (selectedType === "receipt" ? "food" : selectedType === "bill" ? "energy" : selectedType === "product" ? "shopping" : "transport"),
      ...
      console.warn("[Client-side Resilience] Network error during scan submit. Activating local visual fallback estimate engine.", error);
        ...
        {activeScantype === "hub" && (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4" id="scan-tools-grid">
              {[
                { id: "receipt", title: t("scan_hub_receipt", "Receipt Scanner"), desc: t("scan_desc_receipt", "Upload bills & receipts"), icon: Receipt, color: "bg-emerald-50 text-emerald-600" },
                { id: "camera", title: t("scan_hub_camera", "Carbon Camera"), desc: t("scan_desc_camera", "Scan objects & get insights"), icon: Camera, color: "bg-blue-50 text-blue-600" },
                { id: "bill", title: t("scan_hub_bill", "Bill Analyzer"), desc: t("scan_desc_bill", "Electricity, Gas, Fuel Bills"), icon: FileText, color: "bg-purple-50 text-purple-600" },
                { id: "product", title: t("scan_hub_product", "Product Scanner"), desc: t("scan_desc_product", "Check product carbon impact"), icon: ShoppingBag, color: "bg-amber-50 text-amber-600" }
              ].map((tool) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-2xl ${tool.color} flex items-center justify-center`}>
            ...
            <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm">
              <h3 className="font-bold text-xs text-slate-400 uppercase tracking-widest mb-3">Quick Demo Presets</h3>
              <p className="text-xs text-slate-500 mb-4 font-normal">Don't have an image on your device? Tap a preset to load a sample receipt or engine capture for instant Gemini processing:</p>
                ...
                {SAMPLE_PRESETS.map((p) => (
                  ...
                  >
                    <div className="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center shrink-0">
                      {p.type === "receipt" ? <Receipt className="w-4 h-4" /> : p.type === "bill" ? <FileText className="w-4 h-4" /> : <Camera className="w-4 h-4" />}
                    </div>
                    <div>
            ...
            <div className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm space-y-4">
              ...
              {scans.length === 0 ? (
                <div className="text-center py-6 text-slate-400 border border-dashed rounded-2xl">
                  ...
                  <p className="text-[10px] mt-0.5 leading-normal">Your environmental OCR logs will appear here</p>
                </div>
              ) : (
                <div className="space-y-2.5 max-h-[300px] overflow-y-auto pr-1">
                  {scans.map((scan) => {
                    const IconComp = scan.type === "receipt" ? Receipt : scan.type === "bill" ? FileText : (scan.type === "camera" && scan.category === "shopping") ? ShoppingBag : Camera;
                    return (
                      <div 
                        key={scan.id}
                      ...
                      >
                        <div 
                          ...
                          onClick={() => {
                            setSelectedType(scan.type === "camera" && scan.category === "shopping" ? "product" : scan.type);
                            setScanResult({
                              ...scan,
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <h5 className="text-[11px] font-bold text-slate-800 truncate leading-tight">{scan.name}</h5>
                            <p className="text-[9px] text-slate-400 font-mono mt-0.5">{scan.timestamp || "Past scan"}</p>
                          </div>
                        </div>
        ...
        {["receipt", "camera", "bill", "product"].includes(activeScantype) && (
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-800 capitalize">{activeScantype} Scanner</h3>
                <p className="text-[11px] text-slate-400 font-medium leading-none">Power analyzed via Gemini-3.5-flash</p>
              </div>
            </div>
            ...
            <div className="bg-white border rounded-3xl p-6 shadow-sm flex flex-col items-center justify-center relative overflow-hidden min-h-[290px] border-dashed border-slate-200">
              ...
              {!selectedFile ? (
                <div className="flex flex-col items-center text-center">
                  ...
                  <h4 className="font-bold text-sm text-slate-800">Select image from file system</h4>
                  <p className="text-xs text-slate-400 max-w-xs mt-1.5 leading-relaxed">
                    Upload camera roll photos, billing screenshot statements, or snapshot retail grocery sheets.
                  </p>
          ...
          >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-semibold tracking-wider uppercase">
                      {scanResult.isFallback ? "Backup Offline Service" : scanResult.isDemo ? "Simulated OCR data" : "Gemini Core Vision"}
                    </p>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-4">
              ...
              <div className="flex justify-between items-center bg-slate-50/50 p-4 rounded-2xl border">
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-none">Result Score</p>
                  <p className="text-3xl font-extrabold text-slate-850 mt-1">{scanResult.score || scanResult.confidence || 83}%</p>
                  <p className="text-[10px] text-slate-500 mt-1 leading-snug font-medium">Eco Sustainability rating</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-none">Estimated CO2e</p>
              ...
              {/* Textual summary */}
              <p className="text-xs text-slate-600 leading-relaxed font-normal bg-brand-50/30 p-3.5 rounded-2xl border border-brand-50">{scanResult.summary}</p>
            </div>
            ...
            {selectedType === "receipt" && scanResult.detectedItems && (
              <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-sm space-y-4">
                <h4 className="font-extrabold text-xs text-slate-400 uppercase tracking-wider">OCR Scanned Items Baseline</h4>
                  ...
                  {scanResult.detectedItems.map((item: any, i: number) => (
                    <div key={i} className="py-3 first:pt-0 last:pb-0">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs font-bold text-rose-500 font-mono">+{item.co2} kg CO₂</span>
                      </div>
                      <div className="flex items-center justify-between mt-1 pt-1 bg-slate-50/40 p-2 rounded-xl">
                        <span className="text-[10px] font-semibold text-slate-400">Better Choice:</span>
                        <span className="text-[10px] font-bold text-emerald-600">{item.alternative}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {selectedType === "product" && (scanResult.packagingMaterials || scanResult.objectDetected) && (
              <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-sm space-y-4">
                <h4 className="font-extrabold text-xs text-slate-400 uppercase tracking-wider">Product Specs Baseline</h4>
                <div className="space-y-3">
                  <div className="p-3.5 bg-slate-50/50 rounded-2xl border border-slate-100/50 flex justify-between items-center">
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Detected Product</span>
                      <p className="text-xs font-extrabold text-slate-800 mt-1">{scanResult.objectDetected || "Consumer Good"}</p>
                    </div>
                  ...
                  {scanResult.packagingMaterials && (
                    <div className="p-3.5 bg-slate-50/50 rounded-2xl border border-slate-100/50">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Packaging Materials</span>
                      <p className="text-xs font-semibold text-slate-700 mt-1 leading-normal">{scanResult.packagingMaterials}</p>
                    </div>
                  )}
                  ...
                  {scanResult.carbonFootprint && (
                    <div className="p-3.5 bg-slate-50/50 rounded-2xl border border-slate-100/50 flex justify-between items-center">
                      <div>
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Catalog Carbon Footprint</span>
                        <p className="text-xs font-semibold text-slate-700 mt-1 leading-none">{scanResult.carbonFootprint}</p>
                      </div>
            ...
            {((scanResult.personalizedSuggestions && scanResult.personalizedSuggestions.length > 0) || 
              (scanResult.suggestions && scanResult.suggestions.length > 0)) && (
              <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-5" id="ai-carbon-action-generator">
                <div>
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="p-3.5 bg-rose-50/40 rounded-2xl border border-rose-150/40">
                    <span className="text-[9px] font-bold text-rose-500 uppercase tracking-wider block">Impact Score</span>
                    <span className="text-xl font-black text-rose-700 block mt-1">
                      {scanResult.carbonImpactScore || scanResult.score || 85}/100
                    </span>
                    <span className="text-[8px] text-rose-400 font-medium leading-none mt-1 block">Baseline Emission Intensity</span>
                  </div>

                  <div className="p-3.5 bg-emerald-50/40 rounded-2xl border border-emerald-150/40">
                    <span className="text-[9px] font-bold text-emerald-500 uppercase tracking-wider block">Estimated Savings</span>
                    <span className="text-xl font-black text-emerald-700 block mt-1">
                      {scanResult.monthlySavingsEstimate || "₹350/month"}
                    </span>
                    <span className="text-[8px] text-emerald-400 font-medium leading-none mt-1 block">Monthly pocket savings</span>
                  </div>
                </div>

                <div className="p-3.5 bg-brand-50/30 rounded-2xl border border-brand-50 flex items-center justify-between">
                  <div>
                    <span className="text-[9px] font-bold text-brand-600 uppercase tracking-widest">CO2 Reduction potential</span>
                    <p className="text-sm font-extrabold text-slate-800 mt-0.5">{scanResult.carbonReductionPotential || "18kg CO2/month"}</p>
                  </div>
                  <span className="text-xs bg-brand-100 text-brand-700 font-bold px-2.5 py-1 rounded-lg">High Impact</span>
                </div>

                {/* Better alternatives row */}
                {scanResult.betterAlternatives && scanResult.betterAlternatives.length > 0 && (
                  <div className="space-y-2">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Smart Alternatives Proposed</span>
                    <div className="flex flex-wrap gap-1.5">
                      {scanResult.betterAlternatives.map((alt: string, i: number) => (
                        <span key={i} className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1.5 rounded-xl border border-emerald-100/55 shadow-xs">
                          🌱 {alt}
                        </span>
                      ))}
                    </div>
                ...
                <div className="space-y-3">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Interactive Goals & Actionable Tips</span>
                  <div className="space-y-2.5">
                    {(scanResult.personalizedSuggestions || scanResult.suggestions || []).map((s: string, idx: number) => {
                      const isCreated = createdSuggestedGoals[s];
                      return (
                        <div key={idx} className="p-3.5 bg-slate-50/50 hover:bg-slate-50 rounded-2xl border border-slate-100/70 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 transition-all">
                          <div className="flex gap-2.5 items-start">
                            ...
                            <p className="text-xs text-slate-705 leading-normal font-medium">{s}</p>
                          </div>
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try {
                                await addGoal({
                                  title: s,
                                  category: scanResult.category || (selectedType === "receipt" ? "food" : selectedType === "bill" ? "energy" : selectedType === "product" ? "shopping" : "transport"),
                                  targetValue: 25,
